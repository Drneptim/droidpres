...
public class TransferActivity extends Activity implements OnClickListener{
	...
	private void setBtExportEnable() {
		Cursor cur = sDataBase.rawQuery("select count(_id) from document where docstate = " +
	...
	private class ExportToRCD extends AsyncTask<String, String, Void> {
		...
		protected Void doInBackground(String... params) {
			try {
				...
				List<Object> listDocDet = new ArrayList<Object>();
				if (cur_doc.moveToFirst()) {
					do {
						final long _id = QueryHelper.fieldByNameLong(cur_doc, QueryHelper.KEY_ID);
						mapDoc.clear();
						mapDoc.put("_id", _id);
						mapDoc.put("agent_id", sAgentId);
						mapDoc.put("presventype",  QueryHelper.fieldByNameInt(cur_doc, "presventype"));
						mapDoc.put("client_id", QueryHelper.fieldByNameInt(cur_doc, "client_id"));
						mapDoc.put("docdate", QueryHelper.fieldByNameString(cur_doc, "docdate"));
						mapDoc.put("doctime", QueryHelper.fieldByNameString(cur_doc, "doctime"));
						final String desc = QueryHelper.fieldByNameString(cur_doc, "description");
						if (desc.length() > 0) mapDoc.put("description", desc);
						mapDoc.put("paytype", QueryHelper.fieldByNameInt(cur_doc, "paytype"));
						mapDoc.put("paytype1or2", QueryHelper.fieldByNameInt(cur_doc, "paytype1or2"));
						mapDoc.put("paymentdate", QueryHelper.fieldByNameString(cur_doc, "paymentdate"));
						mapDoc.put("typedoc_id", QueryHelper.fieldByNameInt(cur_doc, "typedoc_id"));
						mapDoc.put("itemcount", QueryHelper.fieldByNameFloat(cur_doc, "itemcount"));
						mapDoc.put("mainsumm", QueryHelper.fieldByNameFloat(cur_doc, "mainsumm"));
						...
						if (cur_docdet.moveToFirst()) {
							do {
								...
								listDocDet.add(mapDocDet);
							} while (cur_docdet.moveToNext());

							Object[] XMLRPCDocParams = {mapDoc, listDocDet};
